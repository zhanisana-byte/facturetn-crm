...
export default function InvoiceSignatureClient({
  ...
  async function start() {
    ...
    try {
      ...
      if (!r.ok || !j?.ok) {
        if (j?.need_identity || j?.error === "IDENTITY_MISSING") {
          setMsg({
            ok: false,
            text:
              "Identité DigiGo non configurée. Allez dans Paramètres TTN > Signature DigiGo et enregistrez votre téléphone ou email.",
          });
          setStep("error");
          return;
        }
        setMsg({ ok: false, text: j?.error || j?.message || "Erreur DigiGo." });
        setStep("error");
        return;
      }

      setOtpId(String(j.otp_id || ""));
      setStep("pin");
    } catch (e: any) {
  ...
  async function confirmPin() {
    ...
    try {
      ...
      const j = await r.json().catch(() => null);
      if (!r.ok || !j?.ok) {
        setMsg({ ok: false, text: j?.error || j?.message || "PIN invalide." });
        return;
      }
  ...
  async function confirmOtp() {
    ...
    try {
      ...
      const j = await r.json().catch(() => null);
      if (!r.ok || !j?.ok) {
        setMsg({ ok: false, text: j?.error || j?.message || "OTP invalide." });
        return;
      }
  ...
  return (
    <div className="ftn-page">
      <div className="ftn-page-head">
        <div>
          <h1 className="ftn-h1">Signature facture</h1>
          <p className="ftn-subtitle">Signature DigiGo</p>
        </div>
      ...
      <div className="ftn-card p-6">
        ...
        {msg ? (
          <div className={`mt-4 rounded-xl border p-3 text-sm ${msg.ok ? "border-emerald-200 bg-emerald-50" : "border-rose-200 bg-rose-50"}`}>
            {msg.text}
          </div>
        ) : null}
        ...
        {step === "pin" ? (
          <div className="mt-6 space-y-3">
            ...
            <input className="ftn-input" value={pin} onChange={(e) => setPin(e.target.value)} placeholder="Votre PIN DigiGo" type="password" />
        ...
        {step === "otp" ? (
          <div className="mt-6 space-y-3">
            ...
            <input className="ftn-input" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="Code reçu par SMS / Email" inputMode="numeric" />
            <div className="flex gap-3">
        ...
        {step === "error" ? (
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <a className="ftn-btn ftn-btn-ghost" href="/settings">
              Paramètres
            </a>
          </div>
        ) : null}
