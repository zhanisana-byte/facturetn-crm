...
export default function NewInvoiceClient() {
  ...
  // ✅ Timbre fiscal: valeur par défaut (Tunisie: souvent 1.000 TND)
  ...
  useEffect(() => {
    (async () => {
      ...
      // ✅ Always scope by authenticated user (avoid leaking a global memberships list)
      const { data: auth, error: authErr } = await supabase.auth.getUser();
      const uid = auth?.user?.id;
      if (authErr || !uid) {
        setLoadErr("Non authentifié.");
        return;
      }
      ...
      // ✅ Business rule (Sana): facturation seulement sur les sociétés dont je suis OWNER
      ...
      <div className="ftn-inv-wrap">
        <div className="ftn-card">
          <div className="ftn-inv-head">
            <div>
              <div className="ftn-inv-h">Nouvelle facture</div>
              <div className="ftn-inv-sub">
                Société : <b>{companyName}</b> — Choisis le mode de création.
              </div>
            </div>
          ...
          {companies.length > 1 && (
            <div className="mt-1">
              ...
              <div className="ftn-muted mt-2">
                Astuce : si tu ne vois pas une société ici, vérifie l’accès dans <b>Récap accès</b>.
              </div>
            </div>
          )}


          <div className="mt-4 grid gap-3 rounded-2xl border border-slate-200 bg-white/60 p-4">
            <div className="text-sm font-semibold text-slate-900">Quel mode choisir ?</div>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-white/70 p-3">
                <div className="text-sm font-semibold">Facture normale</div>
                <ul className="mt-2 space-y-1 text-sm text-slate-700">
                  <li>• 1 seule fois</li>
                  <li>• Idéal pour vente / prestation</li>
                  <li>• Export PDF &amp; XML</li>
                </ul>
              </div>
              <div className="rounded-xl border border-slate-200 bg-white/70 p-3">
                <div className="text-sm font-semibold">Facture permanente</div>
                <ul className="mt-2 space-y-1 text-sm text-slate-700">
                  <li>• Modèle récurrent (mensuel)</li>
                  <li>• Idéal pour loyer / maintenance</li>
                  <li>• Génération mensuelle + validation</li>
                </ul>
              </div>
            </div>
          </div>

          <div className="mt-4 ftn-mode-grid2">
            <button className="ftn-mode-pro ftn-mode-pro--normal" onClick={() => setInvoiceMode("normal")}>
              <div className="ftn-mode-top">
                <div className="ftn-mode-dot ftn-dot-orange" />
                <div className="ftn-mode-txt">
                  <div className="ftn-mode-title">Facture normale</div>
                  <div className="ftn-mode-desc">
                    Pour une facture unique : création immédiate, export PDF/XML ensuite.
                  </div>
                </div>
                <div className="ftn-mode-radio" aria-hidden />
              </div>
              <div className="ftn-mode-pills">
                <span className="ftn-pill">Rapide</span>
                <span className="ftn-pill">1 seule fois</span>
                <span className="ftn-pill">PDF / XML</span>
              </div>
              <div className="ftn-mode-foot">
                <div className="ftn-muted">
                  Recommandé pour la majorité des factures (vente, prestation, avoir, devis).
                </div>
              </div>
            </button>

            <button className="ftn-mode-pro ftn-mode-pro--perm" onClick={() => setInvoiceMode("permanente")}>
              <div className="ftn-mode-top">
                <div className="ftn-mode-dot ftn-dot-blue" />
                <div className="ftn-mode-txt">
                  <div className="ftn-mode-title">Facture permanente</div>
                  <div className="ftn-mode-desc">
                    Pour les factures mensuelles : on enregistre un modèle récurrent.
                  </div>
                </div>
                <div className="ftn-mode-radio" aria-hidden />
              </div>
              <div className="ftn-mode-pills">
                <span className="ftn-pill">Mensuelle</span>
                <span className="ftn-pill">Modèle</span>
                <span className="ftn-pill">Automatique</span>
              </div>
              <div className="ftn-mode-foot">
                <div className="ftn-muted">
                  Exemple : loyer, abonnement, maintenance, honoraires récurrents.
                </div>
              </div>
            </button>
          </div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(e) => {
                      ...
                      setStampEnabled(on);
                      if (on && (!stampAmount || Number(stampAmount) === 0)) setStampAmount(1);
                    }}
              />
