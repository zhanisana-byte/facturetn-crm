...
import ModeEnvoiManualClient from "./ui";
...
export default async function ModeEnvoiManualPage({ params }: Props) {
  ...
  const { data: cred } = await admin.from("ttn_credentials").select("*").eq("company_id", companyId).eq("environment", env).maybeSingle();
  ...
  return (
    <div className="p-6">
      <ModeEnvoiManualClient company={company} initial={safeInitial} />
    </div>
  );
}
