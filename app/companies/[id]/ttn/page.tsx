...
export default async function CompanyTTNSettingsHome({ params }: PageProps) {
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <div className="text-xs text-slate-500 mt-2">
          Configurez la méthode d’envoi TTN et la signature. Chaque section s’ouvre dans une page séparée.
        </div>
      </div>
        ...
        <a href={`/companies/${companyId}/ttn/mode-envoi`} className="rounded-2xl border p-5 hover:bg-slate-50 transition">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="text-sm opacity-70 mt-1">Choisissez l’envoi direct (API) ou la déclaration manuelle.</div>
        ...
        <a href={`/companies/${companyId}/ttn/signature`} className="rounded-2xl border p-5 hover:bg-slate-50 transition">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="text-sm opacity-70 mt-1">Choisissez DigiGO, Clé USB (agent local) ou aucune signature.</div>
              <div className="mt-3 text-sm">
                <span className="font-medium">Sélection actuelle :</span>{" "}
      ...
      <div className="pt-2">
        <a className="ftn-btn-ghost" href={`/companies/${companyId}`}>
          ← Retour Société
        </a>
      </div>
    </div>
