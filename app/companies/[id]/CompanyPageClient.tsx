...
type TtnSettings = { id: string } | null;
...
export default function CompanyPageClient({ companyId }: { companyId: string }) {
  ...
  async function load() {
    ...
    // TTN status (best-effort)
    const { data: ttn, error: ttnErr } = await supabase
      .from("company_ttn_settings")
      .select("id")
      .eq("company_id", companyId)
      .maybeSingle<TtnSettings>();

    if (!ttnErr) setTtnOk(!!ttn?.id);
    else setTtnOk(null);

    // Invoice count (best-effort)
  ...
  return (
    <div className="ftn-grid">
      <div className="ftn-grid-3">
        ...
        <LuxCard title="TTN" subtitle="Paramètres TTN / TEIF" delay={60}>
          <div className="text-sm text-slate-600">
            Configure les champs TTN (TEST/PROD), requis pour l’envoi et la conformité.
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
