...
function roleLabel(m: Member) {
  // "Admin" = staff avec permissions fortes (UI label فقط)
  const isAdmin =
    m.role === "staff" &&
    m.can_manage_customers &&
    m.can_create_invoices &&
    m.can_validate_invoices &&
    m.can_submit_ttn;
  ...
  return (
    <div className="mx-auto w-full max-w-6xl p-6 space-y-4">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold">Rôles & Permissions — Société</h1>
        ...
        {!canEdit ? (
          ...
          <div className="text-xs text-slate-500">Owner: يمكنك تعديل الرول والصلاحيات + تعطيل الوصول.</div>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100">
          ...
          <div className="text-xs text-slate-500">Role + permissions (Clients / Factures / Validation / TTN)</div>
        </div>
                ...
                <div key={m.id} className="p-4">
                        ...
                        <>
                          <label className="text-xs text-slate-600">
                            ...
                            <select
                              ...
                              value={m.role}
                              onChange={(e) => setMember(m.id, { role: e.target.value as any })}
                            >
                          ...
                          <label className="text-xs text-slate-600 inline-flex items-center gap-2">
                            <input
                              ...
                              checked={isAdmin}
                              disabled={disabled || isSaving}
                              onChange={(e) => promoteAdmin(m.id, e.target.checked)}
                            />
                            Admin (full)
                          </label>
                        </>
                  ...
                  <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                    {[
                      ...
                      <div key={key} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 flex items-center justify-between">
                        ...
                        {canEdit ? (
                          <input
                            ...
                            onChange={(e) => setMember(m.id, { [key]: e.target.checked } as any)}
                          />
                        ) : (
