...
export async function POST(req: Request) {
  ...
  const invoice_id = invoice_id_raw && isUuid(invoice_id_raw) ? invoice_id_raw : null;

  const credentialId = s(body.credentialId || body.digigo_signer_email || process.env.DIGIGO_CREDENTIAL_ID || "");
  if (!credentialId) return NextResponse.json({ ok: false, error: "CREDENTIAL_ID_MISSING" }, { status: 400 });

  const hashBase64 = s(body.hashBase64 || body.hash || "");
  const safeHash = hashBase64 || "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=";
  ...
  if (ins.error) {
    return NextResponse.json({ ok: false, error: "SESSION_CREATE_FAILED", message: ins.error.message }, { status: 500 });
  }
