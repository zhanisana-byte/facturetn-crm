...
function normalizeEnv(v: string): DigigoEnv {
...
export async function POST(req: Request) {
  try {
    ...
    const env = normalizeEnv(
      process.env.DIGIGO_ENV ||
        process.env.NEXT_PUBLIC_DIGIGO_ENV ||
    ...
    const credRes = await svc
      ...
      .select("signature_provider, signature_config, signer_email, cert_email, updated_at, environment, is_active")
    ...
    const provider = s(cred?.signature_provider);

    if (provider && provider !== "digigo") {
    ...
    const credentialId =
      ...
      s(signatureConfig.digigo_credential_id) ||
      s(signatureConfig.digigoCredentialId) ||
    ...
    try {
      ...
      return NextResponse.json(
        { ok: false, error: "AUTHORIZE_URL_FAILED", details: s(e?.message), env, hasClientId: !!clientId, hasRedirectUri: !!redirectUri },
        { status: 500 }
      );
    }

    const res = NextResponse.json(
      { ok: true, authorize_url, state, invoice_id, back_url: backUrlFinal, environment: env },
      { status: 200 }
    );
    ...
    return NextResponse.json({ ok: false, error: "START_FAILED", details: s(e?.message) }, { status: 500 });
  }
}
