...
import { digigoBaseUrl, digigoClientId, digigoClientSecret, digigoGrantType, digigoAllowInsecure } from "@/lib/digigo/env";
...
export async function POST(req: Request) {
  ...
  const { r: rTok, t: tokText } = await getText(tokenUrl);
  if (!rTok.ok) {
    await svc
