...
async function resolveContext(svc: any, stateIn: string, invoiceIdIn: string) {
  let state = s(stateIn);
  let invoice_id = "";
  let back_url = "";
  ...
  if (state) {
    ...
    return { state, invoice_id, back_url };
  }

  const inv = s(invoiceIdIn);
...
export async function POST(req: Request) {
  try {
    ...
    const { state, invoice_id, back_url } = await resolveContext(svc, stateIn, invoiceIdIn);
