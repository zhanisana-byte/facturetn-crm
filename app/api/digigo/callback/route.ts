...
import { digigoBaseUrl, digigoClientId, digigoClientSecret, digigoGrantType, digigoAllowInsecure, digigoRedirectUri } from "@/lib/digigo/env";
...
export async function POST(req: Request) {
  ...
  if (!invoice_id) {
    return NextResponse.json({ ok: false, error: "MISSING_INVOICE_ID", message: "invoice_id manquant." }, { status: 400 });
  }
  ...
  // ✅ DigiGo peut ne PAS renvoyer state dans la redirection (exemple doc = token only)
  // Donc: si state absent -> on ne bloque pas.
  // Si state présent -> on le valide.
  ...
  if (!digigoCode && token) {
    ...
    if (!jti) return NextResponse.json({ ok: false, error: "JWT_NO_JTI", message: "JWT sans jti." }, { status: 400 });
    digigoCode = jti;
  }
  ...
  // ✅ Doc: oauth2/token/{clientId}/{grantType}/{clientSecret}/{code}
  // + redirectUri OBLIGATOIRE en BODY :contentReference[oaicite:5]{index=5}
  ...
  // Doc: signatures/signHash :contentReference[oaicite:6]{index=6}
  ...
  if (!signResp.r.ok) {
    // fallback au cas où ton environnement expose /signature/...
  ...
  if (!signatureValue) {
    await svc
      .from("invoice_signatures")
      .update({ state: "failed", meta: { ...meta, sign_body: signResp.t } })
      .eq("invoice_id", invoice_id);
