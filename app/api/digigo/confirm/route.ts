...
export async function POST(req: Request) {
  ...
  const invoiceIdBody = s(body?.invoice_id ?? body?.invoiceId ?? body?.id);
  ...
  if (!invoiceId || !isUuid(invoiceId)) return NextResponse.json({ ok: false, error: "INVALID_INVOICE_ID" }, { status: 400 });

  const state = stateParam || stateCookie;
  ...
  if (jti !== state) return NextResponse.json({ ok: false, error: "SESSION_NOT_FOUND" }, { status: 400 });
  ...
  if (s(session.invoice_id) !== invoiceId) return NextResponse.json({ ok: false, error: "MISSING_CONTEXT" }, { status: 400 });
  ...
  if (!Number.isFinite(exp) || exp < Date.now()) {
    ...
    await service.from("invoice_signatures").update({ state: "expired", error_message: "SESSION_EXPIRED" }).eq("invoice_id", invoiceId);
  ...
  if (currentState === "signed" && s(sig.signed_xml)) {
    const res0 = NextResponse.json({ ok: true, back_url: backUrlBody || s(session.back_url) || backCookie }, { status: 200 });
