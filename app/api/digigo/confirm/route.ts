...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (session?.id) {
      ...
      await service
        .from("digigo_sign_sessions")
        .update({ updated_at: new Date().toISOString(), digigo_jti: code })
        .eq("id", session.id);
    }

    step = "read_signature_context";
    ...
    step = "update_invoice_signatures";
    await service
      .from("invoice_signatures")
      .update({
        company_id,
