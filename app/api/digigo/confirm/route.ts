...
function jwtGetSub(token: string): string {
  const t = s(token);
  const parts = t.split(".");
  if (parts.length < 2) return "";
  try {
    const payload = JSON.parse(base64UrlDecodeToString(parts[1]));
    return s(payload?.sub || payload?.SUB || "");
  } catch {
    return "";
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!invoice_id && token) {
      const sub = jwtGetSub(token);
      if (sub) {
        const sigBySub = await service
          .from("invoice_signatures")
          .select("invoice_id, state, meta, updated_at")
          .eq("provider", "digigo")
          .eq("state", "pending")
          .order("updated_at", { ascending: false })
          .limit(10);

        const hit = (sigBySub.data || []).find((x: any) => {
          const meta = x?.meta && typeof x.meta === "object" ? x.meta : {};
          const cred = s(meta?.credentialId || meta?.credential_id || "");
          return cred.toLowerCase() === sub.toLowerCase();
        });

        if (hit?.invoice_id) invoice_id = s(hit.invoice_id);
      }
    }

    if (!session?.id && invoice_id && isUuid(invoice_id)) {
      const sessByInv = await service
      ...
      if (sessByInv.data?.id) {
        session = sessByInv.data;
