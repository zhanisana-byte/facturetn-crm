...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const companyAddress =
    String((sellerCompany as any).address ?? "").trim() ||
    String((sellerCompany as any).address_line ?? "").trim() ||
    String((sellerCompany as any).street ?? "").trim();

  const companyPostal =
    String((sellerCompany as any).postal_code ?? "").trim() ||
    String((sellerCompany as any).zip ?? "").trim();

  const companyCity =
    String((sellerCompany as any).city ?? "").trim() ||
    String((sellerCompany as any).governorate ?? "").trim();
  ...
  const teifXml = buildCompactTeifXml({
    ...
    documentType: String(
      (invoice as any).document_type ??
        (invoice as any).doc_type ??
        ((invoice as any).invoice_type === "credit_note" ? "avoir" : "facture")
    ),
    invoiceNumber: String((invoice as any).invoice_number ?? (invoice as any).number ?? (invoice as any).ref ?? ""),
    issueDate: String((invoice as any).issue_date ?? (invoice as any).date ?? (invoice as any).created_at ?? ""),
    ...
    customer: {
      name: String((invoice as any).customer_name ?? (invoice as any).client_name ?? (invoice as any).customer ?? ""),
      taxId: ((invoice as any).customer_tax_id ?? null) as string | null,
      address: String((invoice as any).customer_address ?? (invoice as any).client_address ?? ""),
      city: String((invoice as any).customer_city ?? (invoice as any).client_city ?? ""),
      postalCode: String((invoice as any).customer_postal_code ?? (invoice as any).client_postal_code ?? ""),
      country: String((invoice as any).customer_country ?? (invoice as any).client_country ?? "TN"),
    },
    totals: {
