...
export async function POST(req: Request) {
  try {
    ...
    if (!tokenJwt)
      return NextResponse.json({ error: "MISSING_TOKEN_JWT" }, { status: 400 });
    ...
    if (!sigState)
      return NextResponse.json(
        { error: "SIGNATURE_META_STATE_MISSING" },
        { status: 400 }
      );

    if (String(sigState) !== String(state))
      return NextResponse.json({ error: "STATE_MISMATCH" }, { status: 400 });

    const unsignedHash = String(sig.unsigned_hash || "").trim();
    const unsignedXml = String(sig.unsigned_xml || "").trim();

    if (!unsignedHash || !unsignedXml)
      return NextResponse.json(
        { error: "MISSING_UNSIGNED_DATA" },
        { status: 400 }
      );
    ...
    const credentialId = String(
      sig?.meta?.credentialId || sig?.meta?.credential_id || ""
    ).trim();

    if (!credentialId)
      return NextResponse.json(
        { error: "MISSING_CREDENTIAL_ID_IN_META" },
        { status: 400 }
      );
    ...
    if (!signatureValue)
      return NextResponse.json(
        { error: "SIGNATURE_EMPTY" },
        { status: 400 }
      );

    const signedXml = unsignedXml.replace(
      "</Invoice>",
      `<Signature>${signatureValue}</Signature></Invoice>`
    );
