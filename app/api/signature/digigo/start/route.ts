...
export async function POST(req: Request) {
  ...
  const { error: upsertError } = await service.from("invoice_signatures").upsert(payload as any, { onConflict: "invoice_id" });
  if (upsertError) {
    return NextResponse.json({ ok: false, error: "SIGNATURE_CONTEXT_INSERT_FAILED", message: upsertError.message }, { status: 500 });
  }

  return NextResponse.json({ ok: true, url: authorize_url, state: stateStr, environment: environment || "test" }, { status: 200 });
}
