...
async function pickEnvForCompany(service: ReturnType<typeof createServiceClient>, companyId: string): Promise<DigigoEnv> {
  // On essaie DIGIGO_ENV si défini
  const desired = (s(process.env.DIGIGO_ENV) === "production" ? "production" : "test") as DigigoEnv;

  const desiredRow = await service
    .from("ttn_credentials")
    .select("environment, signature_provider")
    .eq("company_id", companyId)
    .eq("environment", desired)
    .maybeSingle();

  if (desiredRow.data) return desired;

  // fallback : si production existe, on prend production, sinon test
  const prodRow = await service
    .from("ttn_credentials")
    .select("environment, signature_provider")
    .eq("company_id", companyId)
...
export async function POST(req: Request) {
  try {
    ...
    // Choisir env réel (PROD si ton DB est en prod)
    const env = await pickEnvForCompany(service, company_id);
    ...
    // ✅ Credential : on accepte plusieurs sources (ancien + nouveau)
    const credentialId =
    ...
    if (!credentialId) {
      return NextResponse.json({ ok: false, error: "DIGIGO_CREDENTIAL_MISSING" }, { status: 400 });
    }
    ...
    // Build TEIF XML + hash (ancien comportement)
    ...
    // Create session + cookies (ancien comportement)
