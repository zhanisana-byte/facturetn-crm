...
function parseCompanyId(pathname: string): string | null {
  ...
  const id = m?.[1] ?? null;
  if (!id) return null;
  // ✅ Eviter de traiter /companies/create, /companies/edit, etc. comme un ID
...
function parseGroupId(pathname: string): string | null {
  ...
  const id = m?.[1] ?? null;
  if (!id) return null;
  // ✅ Eviter /groups/invitations, /groups/create, etc.
  const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id);
  return isUuid ? id : null;
}
              ...
              {sidebarItems.map((it) => {
                ...
                return (
                  <Link
                    key={it.key}
                    href={it.href}
                    className={isActive(pathname, it.href, it.exact) ? "ftn-nav-item active" : "ftn-nav-item"}
